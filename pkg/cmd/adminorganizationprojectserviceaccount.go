...
var adminOrganizationProjectsServiceAccountsCreateAPIKey = cli.Command{
	Name:    "create-api-key",
	...
	Action:          handleAdminOrganizationProjectsServiceAccountsCreateAPIKey,
	HideHelpCommand: true,
}
...
func handleAdminOrganizationProjectsServiceAccountsCreateAPIKey(ctx context.Context, cmd *cli.Command) error {
	client := openai.NewClient(getDefaultRequestOptions(cmd)...)
	unusedArgs := cmd.Args().Slice()
	if !cmd.IsSet("project-id") && len(unusedArgs) > 0 {
		cmd.Set("project-id", unusedArgs[0])
	...
	params := openai.AdminOrganizationProjectServiceAccountNewAPIKeyParams{}

	var res []byte
	options = append(options, option.WithResponseBodyInto(&res))
	_, err = client.Admin.Organization.Projects.ServiceAccounts.NewAPIKey(
		ctx,
		cmd.Value("project-id").(string),
		cmd.Value("service-account-id").(string),
		params,
	...
	return ShowJSON(obj, ShowJSONOpts{
		...
		Format:         format,
		RawOutput:      cmd.Root().Bool("raw-output"),
		Title:          "admin:organization:projects:service-accounts create-api-key",
		Transform:      transform,
	})
}
