...
var vectorStoresFileBatchesCreate = requestflag.WithInnerFlags(cli.Command{
	...
	Flags: []cli.Flag{
		...
		&requestflag.Flag[[]string]{
			Name:     "file-id",
			Usage:    "A list of [File](https://developers.openai.com/api/reference/resources/files) IDs that the vector store should use. Useful for tools like `file_search` that can access files. If `attributes` or `chunking_strategy` are provided, they will be applied to all files in the batch. The maximum batch size is 2000 files. This endpoint is recommended for multi-file ingestion and helps reduce per-vector-store write request pressure. Mutually exclusive with `files`.",
			BodyPath: "file_ids",
		},
		&requestflag.Flag[[]map[string]any]{
			Name:     "file",
...
func handleVectorStoresFileBatchesCreate(ctx context.Context, cmd *cli.Command) error {
	...
	return ShowJSON(obj, ShowJSONOpts{
		Context:        ctx,
		Operation:      "(resource) vector_stores.file_batches > (method) create",
...
func handleVectorStoresFileBatchesRetrieve(ctx context.Context, cmd *cli.Command) error {
	...
	return ShowJSON(obj, ShowJSONOpts{
		Context:        ctx,
		Operation:      "(resource) vector_stores.file_batches > (method) retrieve",
		OutputKind:     outputResponse,
		ExplicitFormat: explicitFormat,
		Format:         format,
		RawOutput:      cmd.Root().Bool("raw-output"),
...
func handleVectorStoresFileBatchesCancel(ctx context.Context, cmd *cli.Command) error {
	...
	return ShowJSON(obj, ShowJSONOpts{
		Context:        ctx,
		Operation:      "(resource) vector_stores.file_batches > (method) cancel",
		OutputKind:     outputResponse,
		ExplicitFormat: explicitFormat,
		Format:         format,
		RawOutput:      cmd.Root().Bool("raw-output"),
...
func handleVectorStoresFileBatchesListFiles(ctx context.Context, cmd *cli.Command) error {
	...
	if format == "raw" {
		...
		return ShowJSON(obj, ShowJSONOpts{
			Context:        ctx,
			Operation:      "(resource) vector_stores.file_batches > (method) list_files",
			OutputKind:     outputResponse,
			ExplicitFormat: explicitFormat,
			Format:         format,
			RawOutput:      cmd.Root().Bool("raw-output"),
