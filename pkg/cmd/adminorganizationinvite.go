...
var adminOrganizationInvitesCreate = requestflag.WithInnerFlags(cli.Command{
	...
	Flags: []cli.Flag{
		...
		&requestflag.Flag[[]map[string]any]{
			Name:     "project",
			Usage:    "An array of projects to which membership is granted at the same time the org invite is accepted. If omitted, the user will be invited to the default project for compatibility with legacy behavior. If empty list is passed, the user will not be invited to any projects, including the default one.",
			BodyPath: "projects",
		},
	},
...
func handleAdminOrganizationInvitesCreate(ctx context.Context, cmd *cli.Command) error {
	...
	return ShowJSON(obj, ShowJSONOpts{
		Context:        ctx,
		Operation:      "(resource) admin.organization.invites > (method) create",
...
func handleAdminOrganizationInvitesRetrieve(ctx context.Context, cmd *cli.Command) error {
	...
	return ShowJSON(obj, ShowJSONOpts{
		Context:        ctx,
		Operation:      "(resource) admin.organization.invites > (method) retrieve",
		OutputKind:     outputResponse,
		ExplicitFormat: explicitFormat,
		Format:         format,
		RawOutput:      cmd.Root().Bool("raw-output"),
...
func handleAdminOrganizationInvitesList(ctx context.Context, cmd *cli.Command) error {
	...
	if format == "raw" {
		...
		return ShowJSON(obj, ShowJSONOpts{
			Context:        ctx,
			Operation:      "(resource) admin.organization.invites > (method) list",
			OutputKind:     outputResponse,
			ExplicitFormat: explicitFormat,
			Format:         format,
			RawOutput:      cmd.Root().Bool("raw-output"),
...
func handleAdminOrganizationInvitesDelete(ctx context.Context, cmd *cli.Command) error {
	...
	return ShowJSON(obj, ShowJSONOpts{
		Context:        ctx,
		Operation:      "(resource) admin.organization.invites > (method) delete",
		OutputKind:     outputResponse,
		ExplicitFormat: explicitFormat,
		Format:         format,
		RawOutput:      cmd.Root().Bool("raw-output"),
