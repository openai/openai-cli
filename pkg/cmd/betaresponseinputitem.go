...
var betaResponsesInputItemsList = cli.Command{
	Name:    "list",
	...
	Action:          handleBetaResponsesInputItemsList,
	HideHelpCommand: true,
}

func handleBetaResponsesInputItemsList(ctx context.Context, cmd *cli.Command) error {
	...
	params := openai.BetaResponseInputItemListParams{}
	...
	if format == "raw" {
		...
		_, err = client.Beta.Responses.InputItems.List(
			ctx,
		...
		return ShowJSON(obj, ShowJSONOpts{
			Context:        ctx,
			Operation:      "(resource) beta.responses.input_items > (method) list",
			OutputKind:     outputResponse,
			ExplicitFormat: explicitFormat,
			Format:         format,
			RawOutput:      cmd.Root().Bool("raw-output"),
			Title:          "beta:responses:input-items list",
			Transform:      transform,
		})
	} else {
		iter := client.Beta.Responses.InputItems.ListAutoPaging(
			ctx,
			cmd.Value("response-id").(string),
			params,
			options...,
