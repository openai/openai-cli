...
- Each new Terminal tab has a private gallery with 6,400 image cells (about 12
  square images at the default size). It reports when full instead of replacing
  earlier images. Tabs still displaying a legacy shared gallery retain it so
  upgrading does not change their image scrollback.
- Closed-tab caches can be reclaimed during later previews only after Terminal
  confirms no live tab uses them. If tab inventory fails, cleanup leaves them
  alone. The legacy shared gallery remains available until an explicit reset.
...
Inspect local galleries with `openai images inline status`. It shows the saved
on/off preference, cached image count, approximate remaining square previews,
and disk usage for each gallery without controlling Terminal. Add `--details` for the cache path and exact cell count.
