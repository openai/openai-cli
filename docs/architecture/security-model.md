...
terminal, pager, pipe, or file. The entry point passes the generated command
tree to `custom.Run`. That runtime handles help and completion, validates an
environment-selected base URL before request setup, and formats API errors
([cmd/openai/main.go](../../cmd/openai/main.go),
[pkg/custom/run.go](../../pkg/custom/run.go),
[pkg/cmd/cmd.go](../../pkg/cmd/cmd.go)).
...
| --- | --- | --- |
| `cmd/openai` and `pkg/custom/run.go` | Bootstrap and run the assembled CLI, including help, completion, request setup, and error presentation. | [cmd/openai/main.go](../../cmd/openai/main.go), [pkg/custom/run.go](../../pkg/custom/run.go) |
...
| `pkg/transformers` | Transform one successful JSON response/event at a time; selection depends on the operation and output kind. Explicit presentation choices and errors bypass default transformations. | [pkg/transformers/output.go](../../pkg/transformers/output.go), [pkg/transformers/image.go](../../pkg/transformers/image.go), [pkg/custom/output_transform.go](../../pkg/custom/output_transform.go) |
...
- The CLI must preserve the caller's chosen API operation, path/query/header/body
  ...
  [pkg/custom/cmdutil.go:42-74](../../pkg/custom/cmdutil.go#L42-L74)).
- Local filesystem reads and writes must happen only through intentional CLI
  ...
  [pkg/custom/flagoptions.go:143-289](../../pkg/custom/flagoptions.go#L143-L289)).
- mTLS credentials must be presented only to the explicit same-origin HTTPS
  ...
  ([pkg/custom/mtls.go:78-152](../../pkg/custom/mtls.go#L78-L152)).
...
| --- | --- | --- | --- | --- | --- | --- |
| High | A lower-trust wrapper input influences the request destination while credentials are attached, gaining credential exposure to an unintended backend. | A wrapper forwards attacker-controlled `--base-url` or `OPENAI_BASE_URL`; ordinary operator selection alone is not a bypass. | API/admin credential disclosure or misdirected sensitive requests. | Base URLs require an HTTP(S) prefix; mTLS additionally requires explicit HTTPS and same-origin redirects. | Wrappers should allowlist expected HTTPS origins; preserve SDK redirect/header protections and mTLS origin controls. | [pkg/cmd/cmd.go:39-45](../../pkg/cmd/cmd.go#L39-L45), [pkg/custom/run.go](../../pkg/custom/run.go), [pkg/custom/mtls.go:78-152](../../pkg/custom/mtls.go#L78-L152) |
| High | A lower-trust piped string crosses file-expansion logic and gains a local file read/upload. | Automation forwards untrusted YAML/JSON under trusted-stdin semantics and the process can read the target file. | Disclosure of local files to the selected API backend. | Escaped `\@` is literal; stdin is single-use; `OPENAI_UNTRUSTED_STDIN` preserves provenance and rejects stdin-derived binary paths. | Enable untrusted-stdin mode for lower-trust producers and keep explicit file flags operator-controlled. | [pkg/custom/flagoptions.go:143-289](../../pkg/custom/flagoptions.go#L143-L289), [pkg/custom/flagoptions.go:356-447](../../pkg/custom/flagoptions.go#L356-L447), [pkg/custom/flagoptions.go:608-641](../../pkg/custom/flagoptions.go#L608-L641) |
