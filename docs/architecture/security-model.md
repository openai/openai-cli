...
tree, handles completion, validates an environment-selected base URL before
request setup, and handles API errors with custom presentation hooks
...
| Normal CLI invocation | Request destination | `--base-url` overrides the default; `OPENAI_BASE_URL` is validated before request setup. | Default SDK endpoint or explicit `http://`/`https://` caller-selected URL. | Selected network origin and any configured proxy. | CLI validates URL shape; `openai-go` owns normal transport, redirects, and auth-header behavior. | [pkg/cmd/cmd.go:39-45](../../pkg/cmd/cmd.go#L39-L45), [cmd/openai/main.go](../../cmd/openai/main.go), [pkg/custom/cmdutil.go:33-39](../../pkg/custom/cmdutil.go#L33-L39); default SDK behavior is an external dependency. |
...
| --- | --- | --- | --- | --- | --- | --- |
| High | A lower-trust wrapper input influences the request destination while credentials are attached, gaining credential exposure to an unintended backend. | A wrapper forwards attacker-controlled `--base-url` or `OPENAI_BASE_URL`; ordinary operator selection alone is not a bypass. | API/admin credential disclosure or misdirected sensitive requests. | Base URLs require an HTTP(S) prefix; mTLS additionally requires explicit HTTPS and same-origin redirects. | Wrappers should allowlist expected HTTPS origins; preserve SDK redirect/header protections and mTLS origin controls. | [pkg/cmd/cmd.go:39-45](../../pkg/cmd/cmd.go#L39-L45), [cmd/openai/main.go](../../cmd/openai/main.go), [pkg/custom/mtls.go:78-152](../../pkg/custom/mtls.go#L78-L152) |
