...
| Apple Terminal preview cache | OS user-cache directory under `openai/image-terminal`; one hashed directory per new tab, reclaimed after the tab closes when a later preview safely confirms it is unused; legacy shared galleries remain until reset |
