...
def main():
    ...
    assert before.count("Type: response.output_text.delta") == 2, before
    assert before.count("Hello, streaming world.") == 2, before
    assert after.count("Hello, streaming world.") == 1, after
    assert "Type: response.output_text.delta" not in after, after
    ...
    expected = {"method": "POST", "path": "/v1/responses", "body": {
        "model": "demo-model", "input": "Say hello", "stream": True}}
    ...
    for event in capture[1:]:
        if event[1] != "o":
            continue
        visible += event[2]
        for marker in ("Hello, ", "streaming world.", "Total tokens: 9"):
            if marker not in observed and marker in visible:
                observed[marker] = event[0]
    assert observed["streaming world."] - observed["Hello, "] >= 0.3, observed
    assert observed["Total tokens: 9"] - observed["streaming world."] >= 0.6, observed
