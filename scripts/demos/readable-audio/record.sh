...
demo_prepare_capture "$demo_root" "$1" "$2" "$3" "$4" "$5" \
  "${DEMO_API_BINARY:-$demo_root/dist/demos/bin/audio-demo-api}"
...
cp "$demo_source/readable-audio.tape" "$demo_output/comparison.tape"
...
printf '%s\n\n' "$DEMO_SCENE_LABEL"
sleep 0.25
demo_args=(audio:transcriptions create --file sample.wav --model demo-model --stream=true)
if [ "$DEMO_FORMAT" = jsonl ]; then demo_args=(--format jsonl "${demo_args[@]}"); fi
printf '$ openai'
if [ "$DEMO_FORMAT" = jsonl ]; then printf ' --format jsonl'; fi
printf " audio:transcriptions create --file sample.wav --model demo-model --stream=true\n"
sleep 0.25
...

{
  ...
  echo "data: three fixed synthetic SSE events, 650 ms apart; fake key; loopback only"
  echo "capture: real PTY, stdin/stdout/stderr checked, isolated environment, bash --noprofile --norc"
  echo "render: asciinema 3.2.1 + agg 1.9.0 resvg; Menlo 22px, Dracula, 90 columns x 30 rows, line height 1.2, 20 fps cap"
...
for demo_scene in before after explicit-jsonl; do
  ...
  demo_format=default
  case "$demo_scene" in
    before) demo_command_dir="$demo_runtime/before"; demo_label='Before: main | delayed synthetic transcription';;
    after) demo_label='After: readable audio | identical delayed events';;
  ...
  demo_capture_scene "$demo_scene" 0 "$demo_command_dir" "$demo_api_url" "$demo_label" \
    FORCE_COLOR=0 NO_COLOR=1 "DEMO_FORMAT=$demo_format" "DEMO_FIXTURE_DIR=$demo_runtime"
done
demo_stop_api
...
printf 'Recorded readable-audio terminal replay in %s\n' "$demo_output"
