// Package imagefontmac connects inline images to macOS's font APIs.
// Font registration does not control Terminal. Separate operations update the
// caller's exact tab while preserving its selected profile and size.
