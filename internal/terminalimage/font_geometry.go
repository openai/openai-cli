...
// preservedGeometry leaves the user's point size and source font
...
func preservedGeometry(size fontViewport, pointSize int, source imagefontmac.SourceFont) (imagefont.PreserveOptions, error) {
	...
	if size.PixelWidth != 0 || size.PixelHeight != 0 {
		...
		width = uniqueFontCell(size.Columns, size.PixelWidth, minimumWidth, maximumWidth)
		height = uniqueFontCell(size.Rows, size.PixelHeight, minimumHeight, maximumHeight)
		if width == 0 || height == 0 {
			return imagefont.PreserveOptions{}, geometryError
		}
	}
...
type fontViewport struct {
	Columns, Rows           int
	PixelWidth, PixelHeight int
}

func uniqueFontCell(count, extent, minimum, maximum int) int {
	match := 0
