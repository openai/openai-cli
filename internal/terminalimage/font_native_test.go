...
func TestImageFontRetainsScrollbackAndTextSettings(t *testing.T) {
	...
	require.NoError(t, displayNativeImageFont(t.Context(), &first, img, 32, directory, "/dev/ttys001", testFontViewport, bridge.services(t)))
	...
	require.NoError(t, displayNativeImageFont(t.Context(), &second, img, 32, directory, "/dev/ttys001", testFontViewport, bridge.services(t)))
...
func TestImageFontFailureDoesNotPrintOrCommitImage(t *testing.T) {
	for _, reason := range []string{"denied", "changed"} {
		t.Run(reason, func(t *testing.T) {
			...
			img := image.NewRGBA(image.Rect(0, 0, 16, 16))
			err := displayNativeImageFont(t.Context(), &output, img, 32, directory, "/dev/ttys001", testFontViewport, bridge.services(t))
			require.Error(t, err)
			...
			require.NoError(t, displayNativeImageFont(t.Context(), &output, img, 32, directory, "/dev/ttys001", testFontViewport, bridge.services(t)))
		})
	}
}
...
func TestImageFontOutputFailureDoesNotFallBack(t *testing.T) {
	...
	err := displayNativeImageFont(t.Context(), writerFunc(func([]byte) (int, error) {
		return 0, sinkErr
...
func TestImageFontDetectsNarrowWindowBeforeActivation(t *testing.T) {
	...
	output.Reset()
	err := displayNativeImageFont(t.Context(), &output, img, 32, directory, "/dev/ttys001", func() fontViewport {
